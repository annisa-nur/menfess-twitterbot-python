...
class Twitter:
    ...
    def __init__(self):
        ...
        self.api = tweepy.API(self.inits, wait_on_rate_limit=True, wait_on_rate_limit_notify=True)
        self.me = self.api.me()
            ...
            for x in range(len(dm)):
                ...
                if "attachment" not in json_data:
                    ...
                    if media_type == 'photo':
                        ...
                        print("message will be removed because it contains GIF")
                        print("Deleting dm with id = "+ str(dm[x].id))
                        print("Direct message is empty")
                        try:
                            api.destroy_direct_message(dm[x].id)
                            # kalkulasi waktu disini
                            time.sleep(10)
                        except Exception as ex:
                            print(ex)
    ...
    def post_single_tweet(self, tweet):
        try:
    ...
    def post_single_tweet_with_media(self, tweet, media_url, shorted_media_url, type):
        try:
